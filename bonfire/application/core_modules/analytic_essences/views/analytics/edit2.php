...
<div class="notification success fade-me">
        <div>Шаблон успешно сохранен</div>
</div>
<? } elseif(isset($success) && $success==false) { ?>
<div class="notification error fade-me">
        <div>
            Шаблон не удалось сохранить, проверьте корректность полей
            <?
            //echo "<pre>";
            //print_r($_REQUEST);
            //echo "</pre>";
...
<fieldset style="margin-top: 0pt;width:550px;float:left;">
        ...
        <div>
            ...
            <input name="name" id="name" value="<?=$edittpl->name?>" type="text">
        </div>

        <div>
            <label for="title" class="block">Заголовок шаблона</label>
            <input name="title" id="title" value="<?=$edittpl->title?>" type="text">
        </div>
        ...
        <div>
            ...
            <div class="edtar"><textarea name="tpl" id="tpl"><?=htmlspecialchars($edittpl->tpl)?></textarea></div>
        </div>
    ...
    <div style="float:right;width:190px;border:1px solid #999; margin-top:7px;padding-left:5px;">
