...
class Catalog extends Admin_Controller {
	...
	public function edititemparams()
	{
        ...
        Template::set('toolbar_title', 'Параметры товаров каталога - '.$catalog->title);
        if($this->input->post('add_new_catalog_item_param') && $catalog->id)
        {

                $existsquery = $this->db->get_where('catalog_itemparams', array('name' =>  $this->input->post('param_name'),'catalog_id'=>$catalog->id));
				$param_exist = $existsquery->num_rows();
	            ...
	            { // не будем добавлять не корректные поля в БД
	                ...
	                if(count($errors)<=0)
	                {
		                ...
		                $data = array(
						 ...
						 $this->db->insert('catalog_itemparams', $data);
	                }
        ...
        if($this->input->post('save_catalog_param_data'))
        {
             if(is_array($this->input->post('param_title')))
             {
             	...
             	foreach($param_title as $id=>$v)
             	{
		            ...
		            $this->db->query("UPDATE ".$this->db->dbprefix."catalog_itemparams SET title=?, pos=?, deleted=?, hidden=? where id=?", array($title,$pos,$deleted,$hidden,$id));
             	}

				$q = $this->db->query("SELECT p.*,f.path FROM ".$this->db->dbprefix."catalog_itemparams p LEFT JOIN ".$this->db->dbprefix."recfiles f ON ( f.id = p.recfile_id )  WHERE deleted =1");
				if($q->num_rows()>0)
				{
					foreach($q->result() as $param){
                          if($param->path!='')
					...
					$this->db->query("DELETE FROM ".$this->db->dbprefix."catalog_itemparams WHERE deleted=1 ");
				}
        ...
        $catalog_data = array();
        $this->db->order_by('pos','asc');
        $caralog_params_query = $this->db->get_where('catalog_itemparams', array('catalog_id' =>  $this->catalog_id));


        Template::set('catalog_itemparams', $caralog_params_query->result());
        Template::set('catalog_data', $catalog_data);
		Template::render();
	}
